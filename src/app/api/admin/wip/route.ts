...
export async function POST(request: Request) {
  ...
  try {
    const { id, wipBase64 } = await request.json();
    if (!id || !wipBase64) return NextResponse.json({ error: 'Missing data' }, { status: 400 });

    const match = wipBase64.match(/^data:(image\/[a-zA-Z]+);base64,(.+)$/);
    if (!match) return NextResponse.json({ error: 'Invalid image' }, { status: 400 });

    const contentType = match[1];
    const base64Data = match[2];
    const fileName = `wip-${id}-${Date.now()}.${contentType.split('/')[1]}`;
    ...
    try {
      ...
      await resend.emails.send({
        ...
        subject: `🎨 Progress Update: WIP for ${updated.commission_type}`,
        html: `
          <!DOCTYPE html>
          <html>
            <head>
            ...
            <body style="margin: 0; padding: 0; background-color: #FDFCFE;">
              <div style="font-family: 'Inter', sans-serif; padding: 40px 20px;">
                <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 32px; overflow: hidden; box-shadow: 0 20px 50px rgba(168,85,247,0.1); border: 1px solid #F3E8FF;">
                  <div style="background: linear-gradient(135deg, #7E22CE 0%, #C084FC 100%); padding: 48px 40px; text-align: center;">
                    ...
                    <h1 style="color: white; font-family: 'Georgia', serif; font-size: 32px; font-weight: 400; font-style: italic; margin: 0;">Rendering WIP!</h1>
                  </div>
                  <div style="padding: 48px 40px; text-align: center;">
                    <p style="color: #4B5563; font-size: 16px; line-height: 1.8; margin: 0 0 32px 0;">
                      Hi <strong>${updated.client_name}</strong>,<br><br>
                      I've just uploaded a <strong>Work In Progress (WIP)</strong> update for your <strong>${updated.commission_type}</strong>. 
                      The artwork is coming to life and I'd love for you to see the progress!
                    </p>
      ...
      console.error('Failed to send WIP notification email:', emailErr);
    }
